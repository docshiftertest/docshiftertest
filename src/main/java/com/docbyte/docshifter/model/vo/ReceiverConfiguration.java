...
public class ReceiverConfiguration implements Serializable {
	...
	public void setTransformationConfiguration(
			Map<Integer, ModuleConfiguration> transformationConfiguration) {
		// check if configuration is null
		if (transformationConfiguration == null)
			throw new IllegalArgumentException(
					"No transformation configurations specified for receiver configuration! ");

		for (ModuleConfiguration moduleconfig : transformationConfiguration
				.values()) {
			if (moduleconfig == null)
				throw new IllegalArgumentException(
						"One of the transformationconfigurations is not valid! ");

			// check if configuration is part of transformation module
			if (!moduleconfig.getModule().getType().equals("Transformation"))
				throw new IllegalArgumentException(
						"The moduleconfiguration with id "
								+ moduleconfig.getId()
								+ " is not a configuration for a transformation module!");
	...
	public void setReleaseConfiguration(
			Set<ModuleConfiguration> releaseConfiguration) {
		// check if configuration is null
		if (releaseConfiguration == null)
			throw new IllegalArgumentException(
					"No releaseconfiguration specified for receiver configuration! ");

		for (ModuleConfiguration moduleconfig : releaseConfiguration) {
			if (moduleconfig == null)
				throw new IllegalArgumentException(
						"One of the releaseconfigurations is not valid! ");
			...
			if (!moduleconfig.getModule().getType().equals("Release"))
				throw new IllegalArgumentException(
						"The moduleconfiguration with id "
								+ moduleconfig.getId()
								+ " is not a configuration for a release module!");
		}
	...
	public List<Map> jsonGetTransformation() {
		List<Map> transformations = new ArrayList<Map>();
		Map<String, Object> trans = null;
		for (java.util.Map.Entry<Integer, ModuleConfiguration> entry : transformationConfiguration
				.entrySet()) {
			trans = new HashMap<String, Object>();
			trans.put("sequence", entry.getKey().toString());
			trans.put("configuration", entry.getValue());
	 ...
	 * result = prime result + ((releaseConfiguration == null) ? 0 :
	 * releaseConfiguration .hashCode()); result = prime result +
	 * ((transformationConfiguration == null) ? 0 :
	 * transformationConfiguration.hashCode()); return result; }
	 * 
	 * @Override public boolean equals(Object obj) { if (this == obj) return
	 * true; if (obj == null) return false; if (getClass() != obj.getClass())
	 * return false; ReceiverConfiguration other = (ReceiverConfiguration) obj;
	 * if (releaseConfiguration == null) { if (other.releaseConfiguration !=
	 * null) return false; } else if
	 * (!releaseConfiguration.equals(other.releaseConfiguration)) return false;
	 * if (transformationConfiguration == null) { if
	 * (other.transformationConfiguration != null) return false; } else if
	 * (!transformationConfiguration .equals(other.transformationConfiguration))
	...
	public boolean compareTo(Object obj) {
		...
		if (obj instanceof ReceiverConfiguration) {
			if (obj instanceof ReceiverConfiguration) {
				...
				if (this.releaseConfiguration.size() != rc
						.getReleaseConfiguration().size()) {
					equals = false;
				} else {
					for (ModuleConfiguration mc : this.releaseConfiguration) {
						boolean found = false;

						Iterator<ModuleConfiguration> it = rc
								.getReleaseConfiguration().iterator();
				...
				for (ModuleConfiguration mc : transformationConfiguration
						.values()) {
					boolean found = false;

					Iterator<Map.Entry<Integer, ModuleConfiguration>> it = rc
							.getTransformationConfiguration().entrySet()
							.iterator();
					while (it.hasNext() && !found) {
						if (mc.compareTo(it.next())) {
							found = true;
